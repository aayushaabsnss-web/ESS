...
t('getStatus() returns string',    $a->getStatus(),       'active',     'Status getter');
...
t('getStatus() — active → active',        $active->getStatus(),  'active','Active status string');
t('isActive() — status=resolved → false', $resolved->isActive(), false, 'Resolved alert = false');
t('getStatus() — resolved → resolved',    $resolved->getStatus(),'resolved','Resolved status string');
$def=new Alert(['id'=>1,'product_id'=>1,'product_name'=>'T','sku'=>'T','current_qty'=>0,'threshold'=>5,'alerted_at'=>'','resolved_at'=>'','resolved_by_name'=>'']);
t('isActive() — no status → default true',$def->isActive(),true,'Missing status defaults to active');

// ── GROUP 3: Shortfall Calculation ───────────────────────
grp('GROUP 3 — Shortfall Calculation  [shortfall = threshold - current_qty, floor 0]');
t('qty=0  thresh=10 → 10', $mkA(0,10)->getShortfall(),  10, 'All stock depleted');
t('qty=1  thresh=10 → 9',  $mkA(1,10)->getShortfall(),  9,  'One unit remaining');
t('qty=5  thresh=10 → 5',  $mkA(5,10)->getShortfall(),  5,  'Half threshold remaining');
t('qty=9  thresh=10 → 1',  $mkA(9,10)->getShortfall(),  1,  'Just below threshold');
t('qty=10 thresh=10 → 0',  $mkA(10,10)->getShortfall(), 0,  'Exactly at threshold');
t('qty=11 thresh=10 → 0',  $mkA(11,10)->getShortfall(), 0,  'Above threshold — no shortfall');
t('qty=0  thresh=1  → 1',  $mkA(0,1)->getShortfall(),   1,  'Minimum threshold');
t('qty=0  thresh=999→ 999',$mkA(0,999)->getShortfall(), 999,'Large threshold shortfall');
t('qty=100 thresh=5 → 0',  $mkA(100,5)->getShortfall(), 0,  'High stock — no shortfall');

// ── GROUP 4: Threshold Boundary Tests ────────────────────
grp('GROUP 4 — Threshold Boundary Values');
t('thresh=1  qty=0 — triggers',       $mkA(0,1)->isActive(),                               true, 'Zero qty, thresh=1');
t('thresh=1  qty=1 — at threshold',   $mkA(1,1)->getCurrentQty()<=$mkA(1,1)->getThreshold(),true, 'At threshold');
t('thresh=5  qty=4 — below thresh',   $mkA(4,5)->getCurrentQty()<$mkA(4,5)->getThreshold(), true, 'Below threshold');
t('thresh=5  qty=5 — at threshold',   $mkA(5,5)->getCurrentQty()<=$mkA(5,5)->getThreshold(),true, 'At threshold');
t('thresh=5  qty=6 — above thresh',   $mkA(6,5)->getCurrentQty()>$mkA(6,5)->getThreshold(), true, 'Above threshold');
...
t('thresh=999 qty=998 — near max',    $mkA(998,999)->getShortfall(),                       1,    'One unit short');
...
t('getFormattedResolvedAt() — resolved',    $ar->getFormattedResolvedAt(), '02 May 2026 10:00','Resolved date formatted');
t('getResolvedBy() — resolved by Admin',    $ar->getResolvedBy(),          'Admin',             'Resolved by name');
$aa=$mkA(3,10,'active');
t('getFormattedResolvedAt() — not resolved',$aa->getFormattedResolvedAt(), 'Not resolved',      'Unresolved shows Not resolved');
t('getResolvedBy() — active alert → empty', $aa->getResolvedBy(),          '',                  'No resolver for active alert');
